                                   ...
                                   const std::vector<std::string> &known_maps)
{
    ...
    for (std::size_t i = 0; i < order.poses.size(); ++i)
    {
        ...
        {
            out.push_back({Severity::hard, "pose " + std::to_string(i) + " is not finite (" +
    ...
    {
        out.push_back({Severity::hard, "mapId '" + order.map_id +
                                           "' is not among the maps the AGV reports"});
    }
    ...
    if (factsheet.has_value())
    {
        ...
        {
            out.push_back({Severity::hard,
                           "order has " + std::to_string(order.node_count) +
                               " node(s), over the AGV's declared maxArrayLens['order.nodes'] (" +
                               std::to_string(*fs.max_order_nodes) + ")"});
        }
        if (fs.max_order_edges.has_value() && order.edge_count > *fs.max_order_edges)
        {
            out.push_back({Severity::hard,
                           "order has " + std::to_string(order.edge_count) +
                               " edge(s), over the AGV's declared maxArrayLens['order.edges'] (" +
        ...
        {
            out.push_back({Severity::soft,
                           "order sent " + std::to_string(*order.seconds_since_last_order) +
...
bool is_core_action(const std::string &action_type)
{
    ...
    return std::any_of(std::begin(kCore), std::end(kCore),
                       [&](const char *core) { return action_type == core; });
}
                                              ...
                                              const std::optional<ParsedFactsheet> &factsheet)
{
    ...
    const std::string reason = "action '" + action_type +
                               "' is not in the AGV's factsheet (protocolFeatures.agvActions)";
