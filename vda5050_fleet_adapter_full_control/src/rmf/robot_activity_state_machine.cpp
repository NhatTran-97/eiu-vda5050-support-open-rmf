...
RobotActivityStateMachine::RobotActivityStateMachine(rclcpp::Logger logger, Connector &connector,
                                                      std::string robot_name)
  : _logger(std::move(logger)), _connector(connector), _name(std::move(robot_name))
{
}

std::string RobotActivityStateMachine::derive_node_id(const std::string &name,
...
void RobotActivityStateMachine::on_navigate(
...
void RobotActivityStateMachine::on_stop(ConstActivityIdentifierPtr activity)
{
    std::lock_guard<std::mutex> lock(_mutex);
...
void RobotActivityStateMachine::on_action(const std::string &category,
...
RobotActivityStateMachine::ConstActivityIdentifierPtr RobotActivityStateMachine::on_state_update()
{
    ...
    if (_state == State::EXECUTING_ACTION && _action_exec.has_value())
    {
        ...
        if (status == "FINISHED")
        {
            RCLCPP_INFO(_logger, "[%s] action %s finished", _name.c_str(), _action_id.c_str());
        ...
        if (status == "FAILED")
        {
            ...
            _action_exec->error("VDA5050 action reported FAILED");
            _action_exec.reset();
            _action_id.clear();
            _state = State::IDLE;
            return nullptr;
...
RobotActivityStateMachine::State RobotActivityStateMachine::state()
{
    std::lock_guard<std::mutex> lock(_mutex);
    return _state;
}
