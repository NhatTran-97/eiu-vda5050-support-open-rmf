...
// Builds a route order: base node at sequenceId 0, edges on odd ids, route nodes on even ids (no trajectories).
//
// released_count: how many route points (from the base, always released) go out now; nullopt releases everything, a smaller value leaves the rest
// as horizon for a later order update on the same order_id.
//
// Requires non-empty order_id/base_node_id/route/map_id, finite poses and speed limits, and order_update_id following VDA5050's update sequence.
