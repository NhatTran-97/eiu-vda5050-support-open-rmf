...
// Terminal VDA5050 action states.
...
OperatorInterface::OperatorInterface(rclcpp::Node &node, Vda5050Connector &connector, std::map<std::string, RobotHooks> hooks) 
...
void OperatorInterface::poll_pending_init_actions()
{
  std::map<std::string, std::string> finished;
  {
    ...
    for (auto it = _pending_init_actions.begin(); it != _pending_init_actions.end();)
    {
      ...
      if (result.has_value() && is_terminal_action_status(result->first))
      {
