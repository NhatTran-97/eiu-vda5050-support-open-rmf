...
Dialog {
    ...
    // Live-bound, not a one-time snapshot -- re-evaluates whenever
    // root.telemetry changes, so paused/resumed state reflects immediately
    // instead of freezing at whatever it was when the dialog opened.
    ...
    // "" | "pose" | "waypoint" — which field group a map click should fill.
    // The dialog hides itself while picking so clicks reach the map underneath;
    // MapPage.qml re-emits the pick back through mapLoader.item's signals.
    ...
    // User-draggable position + size — 0 height means "fit content".
    // x/y/width/height are plain properties so a drag just keeps overwriting
    // them; they persist across close()/open() (e.g. the pick-on-map flow),
    // so the dialog reopens wherever the operator last left it.
    ...
    contentItem: ColumnLayout {
        ...
        RowLayout {
            ...
            Button {
                ...
                contentItem: Text { text: parent.text; color: C.text; font.pixelSize: 12; font.bold: true
                                     horizontalAlignment: Text.AlignHCenter }
        ...
        ColumnLayout {
            ...
            Layout.leftMargin: 18; Layout.rightMargin: 18
            spacing: 4
            Text { text: "SPEED LIMIT (m/s, blank = no cap)"; font.pixelSize: 9; font.bold: true
        ...
        ColumnLayout {
            ...
            Layout.leftMargin: 18; Layout.rightMargin: 18
            spacing: 4
            Text { text: "RE-LOCALIZE (x, y, yaw rad — RMF frame)"; font.pixelSize: 9; font.bold: true
                   font.letterSpacing: 1.0; color: C.textDim }
            RowLayout {
                Layout.fillWidth: true
                spacing: 6
                TextField {
                    ...
                    color: C.text; font.pixelSize: 13
                    background: Rectangle { radius: 10; color: C.surfaceAlt; border.color: C.border; border.width: 1 }
                }
                TextField {
        ...
        ColumnLayout {
            ...
            Layout.leftMargin: 18; Layout.rightMargin: 18
            spacing: 4
            Text { text: "GO TO WAYPOINT"; font.pixelSize: 9; font.bold: true
                   font.letterSpacing: 1.0; color: C.textDim }
            RowLayout {
                Layout.fillWidth: true
                spacing: 8
                ComboBox {
                    ...
                    background: Rectangle { implicitHeight: 38; radius: 10
                ...
                Button {
                    ...
                    onClicked: {
                        ros.dispatch(cfg.taskCategories.length > 0 ? cfg.taskCategories[0] : "loop",
                                     goToCombo.currentText, 1)
        ...
        Button {
            ...
            text: "CLOSE"
            implicitHeight: 36
            contentItem: Text { text: parent.text; color: C.text; font.pixelSize: 13
                                 horizontalAlignment: Text.AlignHCenter }
            background: Rectangle { radius: 10; color: parent.down ? C.border : C.surfaceAlt
                                     border.color: C.border; border.width: 1 }
            onClicked: {
