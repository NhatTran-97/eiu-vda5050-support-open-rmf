...
def patch_world(world_file):
    ...
    if 'gz-sim-sensors-system' not in content:
        content = content.replace('<plugin filename="libdoor.so"', SENSORS_PLUGIN + '  <plugin filename="libdoor.so"')
    for model in FUEL_MODELS:
        content = content.replace(f'<uri>model://{model}</uri>', f'<uri>https://fuel.gazebosim.org/1.0/OpenRobotics/models/{model}</uri>')
